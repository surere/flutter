...
class MyApp extends StatelessWidget {   // 상태변화가 없는 애
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Flutter Demo Home Page'), // 상수처리(처음 그려놓고 더 이상 수정하지 않는 것)
    );
  }
}

// 시작 클래스가 표시되는 클래스. 카운터 앱 화면 
class MyHomePage extends StatefulWidget { // 상태가 변하는 애, 정보가 변한다
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold( // 메티리얼 디자인 기본 뼈대 위젯
      appBar: AppBar( // 상단 앱바
      ...
      floatingActionButton: FloatingActionButton(
        // 클릭시_incrementCounter() 메서드 실행 (손가락터치)
